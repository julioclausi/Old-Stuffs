...
#Aqui estou criando uma lista das categorias Estudantes, Disciplinas, Professores, Turmas e Matrículas
...
if selecao_usuario.isdigit():
    ...
    #Neste if verifico se o usuário digitou 1, 2, 3, 4 ou 5
        categoria_selecionada = categorias[int(selecao_usuario)-1].upper() #salvo na variável categoria_selecionada a opção da lista de categorias referente à seleção do usuário
        ...
        if selecao_acao.isdigit():
            if int(selecao_acao) <5 and int(selecao_acao) >0:
                acao_selecionada = acoes[int(selecao_acao)-1].upper()
                print (f'\nVocê selecionou a opção de [{acao_selecionada}] na categoria [{categoria_selecionada}]\n')
            else:
                print ('Opção digitada inválida.')
        else:
            print ('Digite apenas números.')
    elif int(selecao_usuario) == 9:
        #Aqui seria a opção sair, mas não fará nada por enquanto
        ...
